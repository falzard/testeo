...
std::string Warden::Penalty(WardenCheck* check /*= NULL*/, uint16 checkFailed /*= 0*/)
{
    ...
    std::string banReason = "Hack/trampa Detectado de";
    bool longBan = false; // 14d = 1209600s
    if (checkFailed)
        switch (checkFailed)
        {
            case 47: banReason += " (FrameXML Firma de chequeo)"; break;
            case 51: banReason += " (LUA DoString Chequeo)"; break;
            case 59: banReason += " (Parche de protección LUA)"; break;
            case 72: banReason += " (Movement State related)"; break;
            case 118: banReason += " (Escalar Paredes)"; break;
            case 121: banReason += " (Parche Antidaño Caida)"; break;
            case 193: banReason += " (Movimiento de Comprobación Unidad)"; break;
            case 209: banReason += " (Inyección de WoWEmuHacker)"; longBan = true; break;
            case 237: banReason += " (MensajeChatAdición)"; break;
            case 246: banReason += " (Parche de Lenguaje)"; break;
            case 260: banReason += " (Impulso de salto)"; break;
            case 288: banReason += " (Parche de Lenguaje)"; break;
            case 308: banReason += " (MensajeChatEnvío)"; break;
            case 312: banReason += " (Salto Físico)"; break;
            case 314: banReason += " (CogerInfoPersonaje)"; break;
            case 329: banReason += " (Escalado Paredes)"; break;
            case 343: banReason += " (PunteroContraseñaInicio)"; break;
            case 349: banReason += " (Parche de Lenguaje)"; break;
            case 712: banReason += " (WS2_32.Send)"; break;
            case 780: banReason += " (IntentoEliminarProtecciónLUA)"; break;
            case 781: banReason += " (Caminar por encima del Agua)"; break;
            ...
            case 785: banReason += " (Parche de salto múltiple)"; break;
            case 786: banReason += " (WPE PRO)"; longBan = true; break;
            case 787: banReason += " (Editor de paquetes rEdoX)"; break;
        }
