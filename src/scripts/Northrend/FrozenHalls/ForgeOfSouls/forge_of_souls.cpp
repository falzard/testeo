    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        if (creature->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP))
        {
            if (creature->GetEntry() == NPC_JAINA_PART1)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "¿Que quieres de mi, mi señora?.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "¿Que quieres de mi, reina Banshee?", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
        }
